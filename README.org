...
From [[https://melpa.org/#/getting-started][MELPA]]:
...
#+begin_src elisp
  (use-package vice-mode
    :straight (vice-mode :host github :repo "gpapadok/vice")
    :config (vice-mode))
#+end_src
...
- ~count~ — an optional repeat, ascending that many levels for delimiter
  pairs; an error is signaled if given to other objects.
- ~operator~ — what to do.
- ~a~ / ~i~ — /around/ (include the delimiters) or /inside/ (the
  contents only), as in Vim's ~da(~ vs ~di(~.
- ~object~ — what to act on.

Type ~?~ at any prompt to show all operators and objects, then re-prompt.
...
| Key         | Object                                            |
|-------------+---------------------------------------------------|
| ~(~ ~)~ ~b~ | the nearest enclosing parentheses                 |
| ~[~ ~]~     | the nearest enclosing square brackets             |
| ~{~ ~}~ ~B~ | the nearest enclosing curly braces                |
| ~m~         | the nearest enclosing pair of any type            |
| ~"~ ~'~ ~q~ | the enclosing string                              |
| ~w~         | a word                                            |
| ~s~         | a symbol                                          |
| ~p~         | a paragraph                                       |
